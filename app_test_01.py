...
def device_confirm(device):
    global path_txt
    if device == 'banana':
        path_txt = 'banana_config.txt'
    elif device == 'orange':
        path_txt = 'orange_config.txt'
    elif device == 'apple':
        path_txt = 'apple_config.txt'
    data['name'] = device
...
def text_update(event):
    ...
    print(f"input: {top_text_data}")

    response = data_response(f"{top_text_data}")
    if response == "accepted":
        bottom_text.insert(tk.END, f" DEVICE ACCEPTED...\n")
       
        process_data_response_1()

    device_process(path_txt)
...
com_var.set("/dev/tty.usbserial-146340")
